...
下面，再让我们来看看继承时的虚函数表是什么样的。假设有如下所示的一个继承关系：
 ...
 ![](Images/虚函数表解析/o_Drawing3.jpg)
 
 请注意，在这个继承关系中，子类没有重载任何父类的函数。那么，在派生类的实例中，其虚函数表如下所示：
 
对于实例：`Derive d`; 的虚函数表如下：

![](Images/虚函数表解析/o_vtable2.JPG)
...
覆盖父类的虚函数是很显然的事情，不然，虚函数就变得毫无意义。下面，我们来看一下，如果子类中有虚函数重载了父类的虚函数，会是一个什么样子？假设，我们有下面这样的一个继承关系。
 ...
 ![](Images/虚函数表解析/o_Drawing4.jpg)
 
为了让大家看到被继承过后的效果，在这个类的设计中，我只覆盖了父类的一个函数：`f()`。那么，对于派生类的实例，其虚函数表会是下面的一个样子：
 
![](Images/虚函数表解析/o_vtable3.JPG)
...
下面，再让我们来看看多重继承中的情况，假设有下面这样一个类的继承关系。注意：子类并没有覆盖父类的函数。
 ...
 ![](Images/虚函数表解析/o_Drawing1.jpg)
 
对于子类实例中的虚函数表，是下面这个样子：

![](Images/虚函数表解析/o_vtable4.JPG)
...
下图中，我们在子类中覆盖了父类的`f()`函数。
 ...
 ![](Images/虚函数表解析/o_Drawing2.jpg)
 
下面是对于子类实例中的虚函数表的图：
 
![](Images/虚函数表解析/o_vtable5.jpg)
